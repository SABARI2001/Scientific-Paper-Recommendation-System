...
class ScientificPaperRecommender:
    ...
    def cluster_papers(self, papers, retrieval_method="tf-idf", clustering_method="K-Means"):
        ...
        if clustering_method == "K-Means":
            ...
            num_clusters = min(5, len(papers))  # You can adjust the number of clusters
        ...
        return self.create_clustered_papers_dict(labels, papers)



    def evaluate_clustering(self, clustered_papers): 
        labels = []
        for cluster_id, paper_list in clustered_papers.items():
            labels.extend([cluster_id] * len(paper_list))
            
            if len(set(labels)) > 1:  # Ensure there is more than one unique label
                silhouette_avg = silhouette_score(self.get_vectors(clustered_papers), labels)
                st.write(f"Silhouette Score: {silhouette_avg}")
            else:
                st.write("Clustering resulted in a single cluster; silhouette score cannot be computed.")
    ...
    def evaluate_answer(self, answer, question):
        ...
        # Tokenized answer and relevant sentences
        answer_words = set(word_tokenize(answer.lower()))
        relevant_words = set(word_tokenize(' '.join(relevant_sentences).lower()))

        # Get the top 3 sentences as relevant sentences based on similarity
        relevant_sentences = [sentences[i] for i in sorted(range(len(similarities)), key=lambda i: similarities[i], reverse=True)[:3]]

        # 3. Precision and Recall
        precision = len(answer_words.intersection(relevant_words)) / len(answer_words) if answer_words else 0
        recall = len(answer_words.intersection(relevant_words)) / len(relevant_words) if relevant_words else 0
        ...
        # 5. Rouge Score
        rouge = Rouge()
        rouge_scores = rouge.get_scores(answer, ' '.join(relevant_sentences))[0]

        # 6. BLEU Score
        bleu_score = sentence_bleu([relevant_words], answer_words)

        # 7. Answer Length
        answer_length = len(answer_words)

        # 8. Coverage (Proportion of relevant words in the answer)
        coverage = len(answer_words.intersection(relevant_words)) / len(relevant_words) if relevant_words else 0
        ...
        return {
            ...
            "rouge_l_f1": round(rouge_scores['rouge-l']['f'], 3),
    ...
    # Main function
    def get_answer_with_evaluation(self, text, question, relevant_sentences):
        answer = self.answer_question(text, question)
        if answer:
            evaluation_metrics = self.evaluate_answer(answer, question, relevant_sentences)
            return answer, evaluation_metrics
...
def main():
    ...
    if option == "Paper Recommendation":
        ...
        if query:
            ...
            if papers:
                # Display papers as they are before ranking
                ...
                if cluster_button:
                    ...
                    recommender.evaluate_clustering(clustered_papers)
                    for cluster_id, paper_list in clustered_papers.items():
                        st.subheader(f"Cluster {cluster_id}")
                        for idx, paper in enumerate(paper_list, 1):  # Serial numbering within clusters
                            st.markdown(f"{idx}. [{paper['title']}]({paper['pdf_url']})")

# Continue with other options as is...
        ...
        if uploaded_file:
            ...
            if st.button("Get Answer"):
                answer, metrics = recommender.get_answer_with_evaluation(text, question, relevant_sentences)
